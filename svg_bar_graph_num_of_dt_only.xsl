...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="numSongs" select="count($songColl//song//phrase[@type = 'dt']/../../../..)"/>
    ...
    <xsl:template match="/">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="{2400}" height="{500}"
            ...
            <g transform="translate(50 {200})">
                <text x="90" y="{(-$phraseMax - 15) * 6}" fill="red">Bar graph of the count of Derogatory Terms
                   mentioned in only the songs containing Detogatory Terms.</text>
                <!--X axis: -->
                ...
                <xsl:for-each select="$songColl//song//phrase[@type = 'dt']/../../../..">
                    <xsl:if test="count(//phrase[@type = 'dt']) ne 0">
                        ...
                        <!-- rectangle -->
                        <rect width="50" height="{count(//phrase[@type='dt']) * 10}"
                            x="{$xPos}" y="{-count(//phrase[@type='dt']) * 10}" stroke="black"
