...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:comment><xsl:apply-templates select="$numSongs"/></xsl:comment>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="{5000}" height="{500}"
            ...
            <g transform="translate(50 {200})">
                <text x="90" y="{(-$phraseMax - 15) * 6}" fill="red">Bar graph of the count of
                    Emotions mentioned all of the songs.</text>
                <!--X axis: -->
                <line x1="0" y1="0" x2="{($numSongs - 3) * ($Xinterval)}" y2="0" stroke-width="3"
                    stroke="black"/>
                <!--Y axis: -->
                ...
                <xsl:for-each select="1 to xs:integer($phraseMax)">
                    <xsl:if test=". mod 2 = 0">
                        ...
                        <line x1="0" y1="{-. * 10}" x2="{($numSongs - 3) * ($Xinterval)}"
                ...
                <xsl:for-each select="$songColl//song">
                    ...
                    <!-- rectangle -->
                    <rect width="50" height="{count(//phrase[@type='emotion']) * 10}" x="{$xPos}"
                        y="{-count(//phrase[@type='emotion']) * 10}" stroke="black"
                ...
                <xsl:variable name="max_xval" select="($numSongs - 3) * ($Xinterval + 1)"/>
