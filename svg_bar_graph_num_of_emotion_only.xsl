...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="numSongs" select="count($songColl//song//phrase[@type = 'emotion']/../../../..)"/>
    <xsl:variable name="phraseMax"
        select="
            max(for $file in $songColl//song
            return
                count($file//phrase[@type = 'emotion']))"/>
    <xsl:variable name="temp2"
        select="$songColl//song[count(//phrase[@type = 'emotion']) eq $phraseMax]"/>
    ...
    <xsl:template match="/">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="{2400}" height="{500}"
            ...
            <g transform="translate(50 {200})">
                <text x="90" y="{(-$phraseMax - 15) * 6}" fill="red">Bar graph of the count of Emotions
                   mentioned in only the songs containing Emotions.</text>
                <!--X axis: -->
                ...
                <xsl:for-each select="$songColl//song//phrase[@type = 'emotion']/../../../..">
                    <xsl:if test="count(//phrase[@type = 'emotion']) ne 0">
                        ...
                        <!-- rectangle -->
                        <rect width="50" height="{count(//phrase[@type='emotion']) * 10}"
                            x="{$xPos}" y="{-count(//phrase[@type='emotion']) * 10}" stroke="black"
                            stroke-width=".5" fill="{$color}"/>
                        <xsl:comment><xsl:apply-templates select="-count(//phrase[@type = 'emotion'])"/></xsl:comment>
