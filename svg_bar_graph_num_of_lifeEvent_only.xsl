...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="numSongs" select="count($songColl//song//phrase[@type = 'lifeEvent']/../../../..)"/>
    <xsl:variable name="phraseMax"
        select="
            max(for $file in $songColl//song
            return
                count($file//phrase[@type = 'lifeEvent']))"/>
    <xsl:variable name="temp2"
        select="$songColl//song[count(//phrase[@type = 'lifeEvent']) eq $phraseMax]"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:comment><xsl:apply-templates select="$numSongs"></xsl:apply-templates></xsl:comment>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="{2400}" height="{400}"
            viewBox="0 0 {2400} {400}">
            <g transform="translate(50 {200})">
                <text x="90" y="{(-$phraseMax - 15) * 6}" fill="red">Bar graph of the count of Life
                    Events mentioned in only the songs containing Life Events.</text>
                <!--X axis: -->
                ...
                <xsl:for-each select="1 to xs:integer($phraseMax)">
                    <xsl:if test=". mod  2= 0">
                        ...
                        <line x1="0" y1="{-. * 10}" x2="{$numSongs * ($Xinterval+1)}" y2="{-. * 10}"
                            stroke="red" stroke-width=".5" stroke-dasharray="30"/>
                    </xsl:if>
                </xsl:for-each>

                <xsl:for-each select="$songColl//song//phrase[@type = 'lifeEvent']/../../../..">
                    <xsl:if test="count(//phrase[@type = 'lifeEvent']) ne 0">
                        <xsl:variable name="Pos" select="position()"/>
                        <xsl:comment><xsl:apply-templates select="$Pos"></xsl:apply-templates></xsl:comment>
                        ...
                        <!-- rectangle -->
                        <rect width="50" height="{count(//phrase[@type='lifeEvent']) * 10}"
                            x="{$xPos}" y="{-count(//phrase[@type='lifeEvent']) * 10}" stroke="black"
                            stroke-width=".5" fill="{$color}"/>
                        <xsl:comment><xsl:apply-templates select="-count(//phrase[@type = 'lifeEvent'])"/></xsl:comment>
