        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="{5000}" height="{400}"
            ...
            <g transform="translate(50 {200})">
                <text x="90" y="{(-$phraseMax - 15) * 6}" fill="red">Bar graph of the count of Life
                    Events mentioned all of the songs.</text>
                <!--X axis: -->
                <line x1="0" y1="0" x2="{($numSongs+2) * ($Xinterval+1)}" y2="0" stroke-width="3"
                    stroke="black"/>
                <!--Y axis: -->
                ...
                <xsl:for-each select="1 to xs:integer($phraseMax)">
                    <xsl:if test=". mod 2 = 0">
                        ...
                        <line x1="0" y1="{-. * 10}" x2="{($numSongs+2) * ($Xinterval+1)}" y2="{-. * 10}"
                ...
                <xsl:variable name="max_xval" select="($numSongs+2) * ($Xinterval + 1)"/>
